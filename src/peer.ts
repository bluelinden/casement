export class Peer {
  ...
  sendAgnostic(isInside: boolean, message: any, actionName?: string) {
  ...
  requestAgnostic(isInside: boolean, message: any, actionName?: string) {
    const here = isInside ? 'inside' : 'outside';
  ...
  handleIncomingAgnostic(event: MessageEvent, isInside: boolean) {
    const here = isInside ? 'inside' : 'outside';
    const there = isInside ? 'outside' : 'inside';
    const target = isInside ? window.parent : this.iFrame!.contentWindow!;
    ...
    switch (event.data.type) {
      ...
      case `casement-message`:
        ...
        if (!didHandle) this.warny(`Casement: Received a message from ${there} but no handler was set for action name ${event.data.actionName}. Make sure to use the ${this.capitalizeFirstLetter(here)}.on() method to set a handler for this request.`, event);
        break;
      ...
      case `casement-request`:
        ...
        else this.warny(`Casement: Received a request from ${there} but no handler was set. Make sure to use the ${this.capitalizeFirstLetter(here)}.on() method to set a handler for this request.`);
        break;
    }
  }

  handleReadinessAgnostic(event: MessageEvent, isInside: boolean) {
    const here = isInside ? 'inside' : 'outside';
    const there = isInside ? 'outside' : 'inside';
